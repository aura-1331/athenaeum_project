    ...
    <template v-else>
      ...
      <section class="kpi-grid">
        <div class="kpi">
          ...
          <span>Works</span>
          <h2>{{ summaryStats.total_accessions || totalItems }}</h2>
        </div>
      ...
      <section class="grid">
        ...
        <div class="panel">
          <div class="panel-header">
            ...
            <RouterLink to="/audit-trail" class="view-all-link">
              View All &rarr;
            </RouterLink>
          </div>
          ...
          <div
            ...
            :key="log.id"
          >
            <div class="timeline" :class="getTimelineClass(log.action_type || log.action)"></div>
            <div class="activity-content">
              <div class="activity-title-row">
                <strong>{{ formatActionTitle(log.action_type || log.action) }}</strong>
                <span class="actor-tag">{{ log.actor_username || 'Operator' }}</span>
              </div>
              <p>{{ log.summary || log.details || getBookTitle(log.serial_no) }}</p>
            </div>
            <small>{{ formatRelativeTime(log.timestamp || log.changed_at) }}</small>
          </div>
        </div>
        ...
        <div class="panel">
          ...
          >
            ...
            <small>{{ formatDate(book.created_at) }}</small>
          </div>
        </div>
        ...
        <div class="panel">
          ...
          >
            ...
            <div class="bar">
              <div
                class="fill"
                :style="{ width: totalItems > 0 ? (lang.count / totalItems * 100) + '%' : '0%' }"
              ></div>
            </div>
        ...
        <div class="panel">
          ...
          <div class="actions">
            <RouterLink to="/create-work">+ New Work</RouterLink>
            <RouterLink to="/create-item">+ New Item</RouterLink>
        ...
        <div class="panel">
          ...
          <div class="status">
            <span>Languages</span>
            <strong>{{ (summaryStats.languages && summaryStats.languages.length) || 0 }}</strong>
          </div>
...
async function loadDashboard() {
  ...
  try {
    const [summaryRes, catRes, auditRes] = await Promise.allSettled([
      axios.get("/dashboard/summary"),
      axios.get("/catalogue/", { params: { limit: 500 } }),
      axios.get("/status_audit/system-logs", { params: { limit: 8 } })
    ])
    ...
    if (catRes.status === "fulfilled") {
      rawCatalogue.value = catRes.value.data?.data || []
    }

    if (auditRes.status === "fulfilled" && auditRes.value.data?.items) {
      recentActivities.value = auditRes.value.data.items
    } else if (summaryStats.value.recent_activity?.length) {
      recentActivities.value = summaryStats.value.recent_activity
    }
  } catch (e) {
...
const newestArrivals = computed(() => {
  return [...rawCatalogue.value]
    .sort((a, b) => (b.serial_no || 0) - (a.serial_no || 0))
    .slice(0, 8)
})
...
function getBookTitle(id) {
  if (!id) return "System Action"
  const book = rawCatalogue.value.find(b => String(b.serial_no) === String(id))
  return book ? book.title : "Catalogue Record"
}
...
function getTimelineClass(action) {
  ...
  if (act.includes("DELETE") || act.includes("DROP") || act.includes("REJECT")) return "danger"
  return ""
}
