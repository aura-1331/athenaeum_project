...
async function checkHealth() {
  ...
  try {
    ...
    const res = await fetch(
      `${baseUrl}/status_audit/ledger/verify`,
      {
        method: 'GET',
        headers: {
    ...
    if (
      ...
      failureReason.value =
        ...
        'PAYLOAD_TAMPERED'
      status.value =
        `Compromised: Node #${brokenNode.value}`
    } else {
      state.value = 'error'
      status.value =
        data.detail || 'Offline'
...
<template>
  ...
  >
    ...
    <span class="status-label">
      {{
        state === 'valid'
          ? `${verifiedNodes} Nodes Intact`
          : status
      }}
    </span>

    <button
      class="verify-mini-btn"
      @click.stop="checkHealth"
      :disabled="loading"
      title="Re-verify Ledger Hash Chain"
    >
      <RefreshCw
        :size="11"
        :class="{ 'spin': loading }"
