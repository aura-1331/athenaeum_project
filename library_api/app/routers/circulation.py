...
async def issue_book(
    ...
    try:
        ...
        cur.execute(
            "SELECT SUM(amount) FROM fines WHERE user_id = %s AND status = 'UNPAID'",
            (borrower_id,)
        )
        ...
        if debt > 0:
            raise HTTPException(
                status_code=400,
                detail=f"Outstanding fines: {debt}"
            )

        cur.execute(
            """
            SELECT availability_status, is_deleted
            FROM public.items
            WHERE serial_no = %s
            """,
            (serial_no,)
    ...
    try:
        cur.execute(
            ...
            SELECT
                ...
                u.name as borrower,
                l.due_date,
                (CURRENT_TIMESTAMP - l.due_date) as delay
            FROM loans l
            JOIN public.items i
